...
function detectLanguage(text){
  ...
  const n = (norm.match(ES_RX) || []).length;
  if(w === 1) return n >= 1 ? 'es' : 'en';
  if(w <= 3)  return n >= 1 ? 'es' : 'en';
  if(w <= 6)  return (n >= 1 && n/w >= 0.06) ? 'es' : n >= 2 ? 'es' : 'en';
  return n / w >= 0.08 ? 'es' : 'en';
}
...
// NUNCA liberamos el micStream físico por mutear o cambiar idioma.
...
muteBtn.addEventListener('click', () => {
  ...
  if(isMuted){
    ...
    // DESMUTE: bajamos el flag y reiniciamos el recognition.
    // El micStream ya estaba vivo, no se pide permiso nuevo.
    muteBtn.classList.remove('muted'); muteLbl.textContent = 'MUTEAR';
    micDot.classList.remove('pause');
